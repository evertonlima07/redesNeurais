...
public class RedeNeural {
	...
	public double[][] LerConjuntoTreinamento() throws FileNotFoundException {
		...
		while (leitor.hasNext()) {
			...
			// valoresEntreVirgulas[1] + valoresEntreVirgulas[2] +
			// valoresEntreVirgulas[3] + valoresEntreVirgulas[4]);
		}
	...
	public String treinar(double[][] conjuntoTreinamento, double[] valoresEsperados) {
		...
		// double erro = 1.0;
		String result = "";
		// Enquanto o erro for maior que 10% e o numero deepocas menor que 10
		// mill
		
		while ((Math.abs(erro) > 0.05) && (epocas < numeroepocas)) {
			for (int i = 0; i < conjuntoTreinamento[0].length; i++) {
				// Entrada da segunda camada é o valor passando na conexção
				// entre a camada atual e a camada seguite
				...
				if (erro != 0) {
					...
					// Calcula o valor do deslocamento combase no erro e no
					// valor de saida
				...
				/*
				 * Responsavel por fazer as retropropagações Processo de
				 * aprendizado ele volta da segunda cama para a primeira fazendo
				 * as devidas correções de acordo com as entradas da segunda
				 * camada e o deslocamento
				 */
				// retropropagarErroPelaSegundaCamada(entradaSegundaCamada,
				// deslocamento);
				// retropropagarErroPelaPrimeiraCamada(conjuntoTreinamento,
				// entradaSegundaCamada, deslocamento, i);
			}
			epocas++;
	...
	private void retropropagarErroPelaPrimeiraCamada(double[][] conjuntoTreinamento, double[] entradaSegundaCamada,
			double deslocamento, int i) {
		for (int j = 0; j < entradaSegundaCamada.length - 1; j++) {
			...
			for (int k = 0; k < conexoesPrimeiraCamada[j].length; k++) {
				if(TAXA_APRENDIZADO != 0){
					conexoesPrimeiraCamada[j][k] += TAXA_APRENDIZADO * sigma * conjuntoTreinamento[k][i];
				}else{
	...
	private void retropropagarErroPelaSegundaCamada(double[] entradaSegundaCamada, double deslocamento) {
		for (int j = 0; j < conexoesSegundaCamada.length; j++) {
			if (TAXA_APRENDIZADO != 0) {
				conexoesSegundaCamada[j] += TAXA_APRENDIZADO * entradaSegundaCamada[j] * deslocamento;
			} else {
				TAXA_APRENDIZADO = 0.5;
				conexoesSegundaCamada[j] += TAXA_APRENDIZADO * entradaSegundaCamada[j] * deslocamento;
			}
